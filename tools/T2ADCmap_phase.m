...
function [T2map, Dmap] = T2ADCmap_phase(img_phase1, img_phase2, LUTs)
...
for ii=1:size(img_phase1,1)
    for jj=1:size(img_phase1,2)
        
        tlak = abs(LUTs1D-abs(img_phase1(ii,jj)));
        [t,ind] = min(tlak);
        T2map(ii,jj)=Ind1D(1,ind,2);

        if T2map(ii,jj) > 0
            T2map_ind(ii,jj) = round(size(LUTs.DESS,2));
        else
            T2map_ind(ii,jj) = ind;
        end
...
for ii=1:size(img_phase1,1)
    for jj=1:size(img_phase1,2)
        ...
        LUTs1D_D = LUTs.theta2(:,T2map_ind(ii,jj));
        tlak = abs((LUTs1D_D-img_phase2(ii,jj)));
        [t,ind] = min(tlak);
        Dmap(ii,jj)=LUTs.index(ind,T2map_ind(ii,jj),1);

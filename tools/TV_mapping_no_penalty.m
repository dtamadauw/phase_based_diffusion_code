...
function [x1_est, x2_est, loss] = TV_mapping_no_penalty(y1, y2, LUTs)
    %y1: phi_H: phase for high gradient moment
    %y2: phi_L: phase for low gradient moment
    ...
    [rows, cols] = size(y1);
    x1 = (0.0/LUTs.x1) * ones(rows, cols); % Initial T2 map
    x2 = (0000e-12/LUTs.x2) * ones(rows, cols); % Initial ADC map Max:3.5000e-09
    [x1, x2] = T2ADCmap_phase(y1, y2, LUTs);
    max_iter = 250;
    tol = 1e-8;
    alpha = 0.1; % Step size
    ...
    alpha2_min = 1e-9; % Smaller minimum step size for x2
    ...
    for iter = 1:max_iter
        ...
        % Calculate residuals
        f1 = wrapToPi(fnval(mdl_theta.sp, xn'));
        f2 = wrapToPi(fnval(mdl_theta2.sp, xn'));
        r1 = f1' - y1(:);
        r2 = f2' - y2(:);
        ...
        if iter > 1
            ...
            if norm(y_bb_vec) > 0
                ...
                alpha_init = 1e-6; % Fallback initial guess
            end
        else
            alpha_init = 1e-6; % Initial guess for first iteration
        end
...
function alpha = backtracking_line_search(x1, x2, grad_x1, grad_x2, alpha_init, ...
    ...
    f_curr = objective_function(x1, x2, y1, y2, params);
...
function f = objective_function(x1, x2, y1, y2, params)
    % Calculate objective function value

    xn = [x1(:) x2(:)];
    f1 = wrapToPi(fnval(params.mdl_theta.sp, xn'));
    f2 = wrapToPi(fnval(params.mdl_theta2.sp, xn'));
    r1 = f1' - y1(:);
    r2 = f2' - y2(:);    

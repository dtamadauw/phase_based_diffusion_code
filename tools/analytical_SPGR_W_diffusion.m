...
function [f_0, f_1, epsilon_eta, beta] = analytical_SPGR_W_diffusion(TR, T1, T2, alpha, C, D, G, T)
%f_0 = analytical_SPGR_W_diffusion(TR, T1, T2, alpha, C, D, G, T)
...
%Relaxation term including diffusion effect
%Appendix S1 Equation 9
X = @(l) (exp(-1*TR/T2)) * exp(-D*((l)*(l)+(l)+1/3)*G*G*T^3);
Y = @(l) (exp(-1*TR/T1)) * exp(-D*(l*l)*G*G*T^3);


[val, L] = min( abs(mod( (1:10000)*C, pi )) );

L = 4;
%Omega matrix
%Appendix S1 Equation 19
Omega = @(l) [X(-l-1)*X(l-1)*(Y(l)*exp(2*J*C*l)-cos(alpha)), -1*X(-l-1)*exp(-2*J*C*l*l)*(1+Y(l)*exp(2*J*C*l))*sin(alpha/2)*sin(alpha/2);...
            X(l-1)*exp(2*J*C*l*l)*(1+Y(l)*exp(2*J*C*l))*sin(alpha/2)*sin(alpha/2), (-1+Y(l)*exp(2*J*C*l)*cos(alpha)) ];
...
epsilon_eta = -1*Omega_L(2,2)/Omega_L(2,1);%(lambda-Omega_L(2,2))/(Omega_L(2,1));%
epsilon = real(epsilon_eta);
eta = imag(epsilon_eta);

%Appendix S1 Equation 28
beta_numerator = (1-Y(0))*M0*sin(alpha);
beta_denominator = (X(-1)-epsilon)*( X(-1)*(cos(alpha)-Y(0)) + epsilon*(1-Y(0)*cos(alpha)) ) - eta*eta*(1-Y(0)*cos(alpha));
beta = beta_numerator/beta_denominator;

%Appendix S1 Equations 25 and 26
f_0 = beta*(eta*X(-1)+ J*(eta*eta - epsilon*(X(-1)-epsilon)));

...
function main(Total_Correction_Factor)
    ...
    #MESE parameters
    TEs_MESE = [15, 30, 60]*1e-3; # Vector of echo times from 10ms to 150ms
    TR_MESE = 500e-3;
    ...
    for kk=1:length(Ds)
        ...
        S_MESE = 1.0 * exp(-TEs_MESE[1]/T2[kk]) * (1.0-exp(-TR_MESE/1000e-3));
        noise_level = Total_Correction_Factor*S_MESE;
    ...
    filename = s = "PBT2_Julia_$Total_Correction_Factor.mat"

...
"""
    analytical_SPGR_W_diffusion(TR, T1, T2, alpha, C, D, G, T)

This is a Julia translation of the `analytical_SPGR_W_diffusion.m` script. It calculates the analytical signal for SPGR sequences with diffusion effects.
...
- `C`: Phase increment [radian]
- `D`: Diffusion coefficient (m^2/s)
- `G`: Gradient Field (Rad/m)
- `T`: Gradient Time (sec)

# Returns
- `f_0`, `f_1`: Signal components
- `epsilon_eta`: Complex parameter
- `beta`: Signal amplitude factor
"""
function analytical_SPGR_W_diffusion(TR, T1, T2, alpha, C, D, G, T)
    J = 1im
    M0 = 1.0

    # Relaxation term including diffusion effect (Appendix S1 Equation 9)
    X(l) = exp(-TR / T2) * exp(-D * (l^2 + l + 1/3) * G^2 * T^3)
    Y(l) = exp(-TR / T1) * exp(-D * l^2 * G^2 * T^3)

    # In the original MATLAB code, L is calculated but then hardcoded to 4.
    L = 4

    # Omega matrix (Appendix S1 Equation 19)
    Omega(l) = [
        X(-l-1)*X(l-1)*(Y(l)*exp(2*J*C*l)-cos(alpha))   -X(-l-1)*exp(-2*J*C*l^2)*(1+Y(l)*exp(2*J*C*l))*sin(alpha/2)^2
        X(l-1)*exp(2*J*C*l^2)*(1+Y(l)*exp(2*J*C*l))*sin(alpha/2)^2    -1+Y(l)*exp(2*J*C*l)*cos(alpha)
    ]

    # Appendix S1 Equation 21
    Omega_L = Matrix{ComplexF64}(I, 2, 2)
    for ii in 1:L
        Omega_L = Omega(ii) * Omega_L
    end

    # Appendix S1 Equation 27
    epsilon_eta = -Omega_L[2, 2] / Omega_L[2, 1]
    epsilon = real(epsilon_eta)
    eta = imag(epsilon_eta)

    # Appendix S1 Equation 28
    beta_numerator = (1 - Y(0)) * M0 * sin(alpha)
    beta_denominator = (X(-1) - epsilon) * (X(-1) * (cos(alpha) - Y(0)) + epsilon * (1 - Y(0) * cos(alpha))) - eta^2 * (1 - Y(0) * cos(alpha))
    beta = beta_numerator / beta_denominator

    # Appendix S1 Equations 25 and 26
    f_0 = beta * (eta * X(-1) + J * (eta^2 - epsilon * (X(-1) - epsilon)))
    f_1 = beta * (eta + J * (X(-1) - epsilon))

    return f_0, f_1, epsilon_eta, beta
end

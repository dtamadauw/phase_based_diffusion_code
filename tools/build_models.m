...
function model = build_models(x1, x2, y_data)
    % Use csapi to create a 2D cubic spline. 
    % According to Source the Spline Toolbox lets you pass a cell array 
    % of knots {x, y} and the data Z to csapi. 
    sp = csapi({x1, x2}, y_data);


    model.sp = sp;
    model.dx1 = fnder(sp, [1 0]);
    model.dx2 = fnder(sp, [0 1]);
end
...
%{
% 1) Create or load 2D data. 
% x and y are vectors defining the grid in each dimension. 
% Z is a 2D array holding values on that grid. 
x = 0:1:5; % e.g., time or some spatial axis 
y = 0:1:4; % e.g., another spatial axis 
[XX, YY] = ndgrid(x, y);
% Example function values on this grid 
Z = sin(XX) + 0.2*cos(YY);
% 2) Use csapi to create a 2D cubic spline. 
% According to Source the Spline Toolbox lets you pass a cell array 
% of knots {x, y} and the data Z to csapi. 
sp = csapi({x, y}, Z);
% 3) Evaluate the spline at any (xq, yq). 
% Here we create a finer grid for evaluation: 
xq = linspace(0,5,50); yq = linspace(0,4,40); [XXq, YYq] = ndgrid(xq, yq);
% fnval returns interpolated values of the spline over the query points. 
Zq = fnval(sp, {xq, yq});
% Zq will have size [length(xq), length(yq)].
% 4) Obtain partial derivatives using fnder. 
% Referencing Source “spline interpolation and its (exact) derivatives,” 
% fnder(sp, [1 0]) gives ∂/∂x, and fnder(sp, [0 1]) gives ∂/∂y, for a 2D spline.
% Partial derivative wrt x: sp_dx = fnder(sp, [1 0]); dZdx = fnval(sp_dx, {xq, yq});
% Partial derivative wrt y: sp_dy = fnder(sp, [0 1]); dZdy = fnval(sp_dy, {xq, yq});
% Now, dZdx and dZdy contain the partial derivatives of the fitted spline % at each point in the (xq, yq) grid.
% 5) Visualization (optional): figure; surf(XXq, YYq, Zq.'); title('Cubic Spline Surface');
%}

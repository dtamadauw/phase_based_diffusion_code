...
function [x1_est, x2_est, loss] = TV_mapping_penalty(y_L, y_H, lambda1, lambda2, beta1, beta2, LUTs)
    %Sig: Complex images
    ...
    y1 = y_L; %phi_H: phase for high gradient moment
    y2 = y_H; %phi_L: phase for low gradient moment
    ...
    %convergence.
    mask_sig = abs(y_L(:,:,1,1)); th_sig = 0.5*multithresh(mask_sig(:));
    mask_sig = mask_sig > th_sig;
    se = strel('disk',5);
    mask_erode = imclose(mask_sig,se);
    mask_sig = 1+0*mask_erode;

    mask_weight = mask_sig;

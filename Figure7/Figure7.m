...
load('../data/phantom_recon_v3.mat');
[T2_PBD, ADC_PBD] = Generate_ADCT2(img_recon, params);
IMG_PBD = abs(img_recon(end:-1:1,:,4)');
ADC_PBD = ADC_PBD(end:-1:1,:,4)';
T2_PBD = T2_PBD(end:-1:1,:,4)';

[T2_SE, IMG_T2SE] = T2calc_SE('../data/phantom_dicom_v3/');
IMG_T2SE = IMG_T2SE(:,:,1);


%%

IMG_SE = dicomread('../data/phantom_SEADC/dwi.dcm');
ADC_SE = dicomread('../data/phantom_SEADC/adc.dcm');


%Resolution for images: 0.937 mm^2
%ROI diameter = 10 mm
ROI_diameter = round(10/0.937);
[vals, stds, mask, roi_pos] = GetROI_Analysis(IMG_PBD, ADC_PBD, ROI_diameter, 4);
[vals_SE, stds_SE, mask_SE, roi_pos_SE] = GetROI_Analysis(IMG_SE, ADC_SE,ROI_diameter,16);

%Figures for ADC

figure;
regression_line_ci(vals_SE, vals, [0 2500]);xlim([0 2000]); ylim([0 2000]); 
axis square;xlabel('SE ADC (\mu mm^2/s)');ylabel('PBD ADC (\mu mm^2/s)');
set(gca, 'fontname', 'Arial', 'FontSize',14,'FontWeight','normal','LineWidth',2);

figure;
imagesc(ADC_PBD, [0 3000]); colormap(gray);axis square;colorbar;
set(gca, 'fontname', 'Arial', 'FontSize',14,'FontWeight','normal','LineWidth',2);
figure;
imagesc(ADC_SE, [0 3000]); colormap(gray);axis square;colorbar;
set(gca, 'fontname', 'Arial', 'FontSize',14,'FontWeight','normal','LineWidth',2);
...
[vals, stds, mask, roi_pos] = GetROI_Analysis(IMG_PBD, T2_PBD,ROI_diameter, 4);
[vals_SE, stds_SE, mask_SE, roi_pos_SE] = GetROI_Analysis(IMG_T2SE, T2_SE,ROI_diameter,28);
regression_line_ci(vals_SE, vals, [0 200]);xlim([0 200]); ylim([0 200]); 
axis square;xlabel('SE T2 (ms)');ylabel('PBD T2 (ms)');
set(gca, 'fontname', 'Arial', 'FontSize',14,'FontWeight','normal','LineWidth',2);


figure;
imagesc(T2_PBD, [0 200]); colormap('navia');axis square;colorbar;
set(gca, 'fontname', 'Arial', 'FontSize',14,'FontWeight','normal','LineWidth',2);

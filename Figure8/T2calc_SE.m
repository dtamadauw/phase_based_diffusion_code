...
function [T2map_SE, raw_img] = T2calc_SE(dicom_dir)

dicom_list = dir([dicom_dir '/*.dcm']);

for ii=1:length(dicom_list)
    FileT2{ii} = [dicom_list(ii).folder '/' dicom_list(ii).name];
end

[temp] = dicomread(FileT2{1});
L2 = size(temp)


ns2 = length(FileT2);
tau2 = [];
rawT2 = zeros(L2(1), L2(2), ns2);

figure;
tiledlayout(2,4,'TileSpacing','tight');
kern = ones(3,3)/9;
for i0=1:length(FileT2)
    
  header = dicominfo(FileT2{i0});
  tau2(i0) = header.EchoTime;
  [temp] = dicomread(FileT2{i0});
  temp = convn(temp, kern, 'same');

  rawT2(:,:,i0) = temp;
  nexttile
  imagesc(rawT2(:,:,i0)); colormap('gray'); axis off;
end
tau2 = tau2(:);
...
maskT2 = abs(rawT2(:,:,1));
thT2 = 0.25 * mean(maskT2(:));
maskT2 = maskT2 > thT2;
T2M0 = rawT2(:,:,1);
for i0=1:ns2
  rawT2(:,:,i0) = maskT2.*(rawT2(:,:,i0))./T2M0;
end
...
for i0 = 1:(L2(1))
  for j0 = 1:(L2(2))
        ...
        if(maskT2(i0,j0) > 0)
          ydata2 = rawT2(i0,j0,:);
          ydata2 = ydata2(:);
          p = polyfit(tau2(:), log((ydata2(:))), 1);
          T2map_SE(i0, j0) = -1/real(p(1));
        end
